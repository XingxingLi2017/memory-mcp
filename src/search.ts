...
export async function searchMemory(
  ...
  return boosted.map((r) => ({ ...r, snippet: compactSnippet(r.snippet, r.source) }));
}

/**
 * Strip metadata noise from snippets to reduce token waste.
 * Memory entries: remove `- ` prefix, `_(source: ...)_`, `— timestamp`, `[ref:...]`.
...
function compactSnippet(text: string, source: string): string {
  ...
  if (source === "memory") {
    ...
    result = result
      .split("\n")
      .map((line) => {
        const m = line.match(/^- (.+?)(?:\s+\[ref:.*?\])?(?:\s+_\(source:.*?\)_)?(?:\s+—\s+\d{4}.*)?$/);
        return m ? m[1]! : line;
      })
      .join("\n");
  }
